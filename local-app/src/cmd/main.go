...
import (
	"fmt"
	adapter2 "mindnoscape/local-app/src/pkg/adapter"
...
func main() {
	...
	adapterManager := adapter2.NewAdapterManager(sessionManager)
	defer adapterManager.Shutdown()

	fmt.Println("DEBUG: Adapter manager initialized.")

	// Initialize cli  adapter
	cliAdapter, err := adapter2.NewCLIAdapter(sessionManager)
	if err != nil {
