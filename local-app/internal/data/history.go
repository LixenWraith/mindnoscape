...
func (hm *HistoryManager) AddToHistory(op Operation) {
...
func (hm *HistoryManager) Undo() error {
	...
	switch op.Type {
	case OpAdd:
		err = hm.mm.NodeManager.DeleteNode(strconv.Itoa(op.AffectedNode.Index), true)
	case OpDelete:
		err = hm.restoreSubtree(op.DeletedTree)
	case OpMove:
		err = hm.mm.NodeManager.MoveNode(strconv.Itoa(op.AffectedNode.Index), strconv.Itoa(op.OldParentID), true)
	case OpModify:
		err = hm.mm.NodeManager.ModifyNode(strconv.Itoa(op.AffectedNode.Index), op.OldContent, op.OldExtra, true)
	}
...
func (hm *HistoryManager) Redo() error {
	...
	switch op.Type {
	case OpAdd:
		err = hm.mm.NodeManager.AddNode(strconv.Itoa(op.AffectedNode.ParentID), op.NewContent, op.NewExtra, true)
	case OpDelete:
		err = hm.mm.NodeManager.DeleteNode(strconv.Itoa(op.AffectedNode.Index), true)
	case OpMove:
		err = hm.mm.NodeManager.MoveNode(strconv.Itoa(op.AffectedNode.Index), strconv.Itoa(op.NewParentID), true)
	case OpModify:
		err = hm.mm.NodeManager.ModifyNode(strconv.Itoa(op.AffectedNode.Index), op.NewContent, op.NewExtra, true)
	}
...
func (hm *HistoryManager) restoreSubtree(nodes []*models.Node) error {
	for _, node := range nodes {
		err := hm.mm.NodeManager.AddNode(strconv.Itoa(node.ParentID), node.Content, node.Extra, true)
		if err != nil {
